...
from .import_parsers import cell, normalize_cj_code, normalize_cp_code, to_str
...
def validate_lignes_rapport(rows: list[dict]) -> ValidationResult:
    result = ValidationResult()
    for idx, row in enumerate(rows, start=2):
        _require(result, 'lignes_rapport.csv', idx, row, 'id_rapport')
        if not to_str(cell(row, 'cuve_journaliere', 'id_cuve_journaliere', 'identifiant')):
            result.add(
                ValidationIssue(
                    'lignes_rapport.csv',
                    idx,
