...
def setup_import_logger() -> logging.Logger:
    ...
    try:
        ...
        # Fallback console handler si écriture fichier impossible
...
class ImportDataService:
    ...
    def run(self) -> tuple[ImportStats, ValidationResult]:
        ...
        rapports_rows = self._load('rapport.csv')
        ...
        with transaction.atomic():
            ...
            if rapports_rows:
                self._import_rapports(rapports_rows)
            if lignes:
    ...
    def _import_sites(self, rows: list[dict]) -> None:
        ...
        for row in rows:
            ...
            Site.objects.update_or_create(
                nom=nom,
                defaults={
                    'code': to_str(cell(row, 'code')),
                    'adresse': to_str(cell(row, 'adresse')),
                    'ville': to_str(cell(row, 'ville', 'localisation')) or '',
                },
            )
            count += 1
    ...
    def _import_rapports(self, rows: list[dict]) -> None:
        count = 0
        for row in rows:
            rapport_id = cell(row, 'id_rapport', 'rapport_id', 'rapport')
            if rapport_id in (None, ''):
                self.stats.warnings.append('Rapport sans id_rapport — ignoré')
                continue
            try:
                rapport_pk = int(str(rapport_id))
            except (TypeError, ValueError):
                self.stats.warnings.append(f'Rapport id invalide « {rapport_id} » — ignoré')
                continue
            d1 = self._parse_date(cell(row, 'date_debut'))
            d2 = self._parse_date(cell(row, 'date_fin'))
            if not d1 or not d2:
                self.stats.warnings.append(f'Rapport {rapport_pk} sans période valide — ignoré')
                continue
            rapport, created = Rapport.objects.update_or_create(
                id=rapport_pk,
                defaults={'date_debut': d1, 'date_fin': d2},
            )
            if not created:
                rapport.date_debut = d1
                rapport.date_fin = d2
                rapport.save(update_fields=['date_debut', 'date_fin'])
            count += 1
        self.stats.rapports = count
    ...
    def _import_lignes(self, rows: list[dict]) -> None:
        # Grouper par rapport via l'identifiant fourni dans le CSV.
        buckets: dict[int | str, list[dict]] = {}
        for row in rows:
            rapport_ref = cell(row, 'id_rapport', 'rapport_id', 'rapport')
            if rapport_ref in (None, ''):
                self.stats.warnings.append('Ligne rapport sans id_rapport — ignorée')
                continue
            buckets.setdefault(str(rapport_ref), []).append(row)

        rapport_index = 0
        for rapport_key, bucket in sorted(buckets.items(), key=lambda item: str(item[0])):
            rapport_ref = str(rapport_key)
            try:
                rapport_pk = int(rapport_ref)
            except (TypeError, ValueError):
                self.stats.warnings.append(f'Rapport id invalide « {rapport_ref} » — lignes ignorées')
                continue

            rapport = Rapport.objects.filter(id=rapport_pk).first()
            if rapport is None:
                self.stats.warnings.append(f'Rapport {rapport_pk} introuvable — lignes ignorées')
                continue
            ...
            self.log.info('Rapport %s (%s→%s) — %s ligne(s)', rapport.id, rapport.date_debut, rapport.date_fin, imported)

            # À partir du 2e rapport, déclencher la détection d'alertes
            # (simule la soumission successive des fiches en production)
            rapport_index += 1
            if rapport_index >= 2 and not self.dry_run:
                try:
                    from apps.services.alerts import detecter_et_persister_alertes

                    result = detecter_et_persister_alertes()
                    self.log.info(
                        'Alertes après rapport %s (%s→%s) : créées=%s mises_à_jour=%s ignorées=%s actives=%s',
                        rapport_index,
                        rapport.date_debut,
                        rapport.date_fin,
                        result['created'],
                    ...
                    self.log.exception(
                        'Échec détection alertes après rapport %s (%s→%s)',
                        rapport_index, d1, d2,
                    )
