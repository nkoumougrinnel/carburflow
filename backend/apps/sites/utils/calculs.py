...
def _site_depotage_from_lines(lines) -> float:
    """
    Dépotage site (algébrique) : positif = apport, négatif = retrait/transfert.
    Si la même valeur est répétée sur chaque ligne groupe, on ne la compte
    qu’une fois ; sinon on somme les mouvements non nuls.
    """
