...
def user_is_viewer(user):
    """Rôle user : consultation des sites uniquement."""
    return get_user_role(user) == UserProfile.ROLE_USER


def user_can_upload_rapports(user):
    """Admin et opérateur peuvent déposer des relevés."""
    role = get_user_role(user)
    return role in (UserProfile.ROLE_ADMIN, UserProfile.ROLE_OPERATEUR)


def user_can_manage_rapports(user):
    """Liste / export des rapports (pas le viewer)."""
    return user_can_upload_rapports(user)
