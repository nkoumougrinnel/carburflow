...
function OperatorHomePage({ onNavigate }) {
  ...
  const actions = [
    { id: 'sites', icon: MapPinned, title: 'Sites', text: 'Consulter les cuves et l’autonomie.', cta: 'Voir les sites', tone: 'sites' },
    { id: 'reports', icon: Upload, title: 'Relevé', text: 'Générer et déposer la fiche de la semaine.', cta: 'Envoyer un relevé', tone: 'reports' },
    { id: 'history', icon: History, title: 'Historique', text: 'Retrouver vos envois passés.', cta: 'Ouvrir l’historique', tone: 'history' },
  ]
    ...
    <div className="app-shell">
      ...
      <PageEnter>
        <main className="op-home">
          <WelcomeBanner subtitle="Résumé de votre espace : sites, relevé et historique." />
          {error && (
          ...
          <section className="op-home-actions" aria-label="Accès rapide">
            {actions.map(({ id, icon: Icon, title, text, cta, tone }) => (
              <button key={id} type="button" className={`op-home-action op-home-action--${tone}`} onClick={() => onNavigate(id)}>
                <span className="op-home-action-icon" aria-hidden="true"><Icon size={22} /></span>
                <span className="op-home-action-body">
                  ...
                  <span className="op-home-action-cta">{cta}<ArrowRight size={16} aria-hidden="true" /></span>
                </span>
              </button>
            ))}
          </section>
          ...
          <div className="op-home-columns">
            <section className="op-home-panel">
              <div className="op-home-panel-head">
                <div><h2>Sites</h2><p>Aperçu et autonomie.</p></div>
                <button type="button" className="reports-btn reports-btn--ghost" onClick={() => onNavigate('sites')}>Tout voir</button>
              </div>
              {sites.length === 0 ? (
            ...
            <section className="op-home-panel">
              <div className="op-home-panel-head">
                <div><h2>Derniers relevés</h2><p>Vos envois récents.</p></div>
                <button type="button" className="reports-btn reports-btn--ghost" onClick={() => onNavigate('history')}>Historique</button>
              </div>
              {recentRapports.length === 0 ? (
                ...
                <ul className="op-home-report-list">
                  {recentRapports.map((r) => (
                    <li key={r.id}>
                      <button type="button" className="op-home-report-row" onClick={() => onNavigate('history')}>
                        <span className="op-home-report-id">n°{r.id}</span>
                        <span className="op-home-report-period">{formatDate(r.date_debut)} → {formatDate(r.date_fin)}</span>
                        <span className="op-home-report-lines">{r.lignes_count ?? 0} ligne{(r.lignes_count ?? 0) > 1 ? 's' : ''}</span>
                      </button>
                    </li>
                  ))}
                </ul>
